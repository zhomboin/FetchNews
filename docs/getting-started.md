...
- Compose ??? `PostgreSQL` ?????????????? `PostgreSQL` ??
- ?????? `host.docker.internal` ????? `PostgreSQL`
- ?????? `PostgreSQL`?`migrate` / `api` / `worker` / `beat` ?????????
- ???????? sqlite ? `APP_DATABASE_URL` ???? `SELECT 1` ????????????????????????
- ? `api`?`worker`?`beat` ???????? migration
- ?? `PostgreSQL` ? `APP_DATABASE_BOOTSTRAP_MODE=skip`?? `auto` ??? `skip`????????????? `alembic upgrade head`???????????

## 环境变量配置

???????? `.env`??? [`.env.example`](/D:/Code/Project/FetchNews/.env.example) ??????
...
- 失败任务可以重试
- ???????????????? `rate_limit`??????????? dispatch ??????? `last_provider_status=rate_limited`??????????????????????????
