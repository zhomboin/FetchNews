...
const metrics = [
  { label: "今日采集", value: "184", note: "+12 高频信号" },
...
const queueItems = [
  ...
  },
  {
    ...
    sources: "Blog · X",
    state: "待确认",
    reason: "缺少第三来源，建议人工补检 release note",
  },
  {
...
const articleSections = [
  ...
  {
    heading: "开源与社区",
    body: "社区端更关注评测基准和真实工作流，热度正在从单纯参数规模，转向部署稳定性、工具链成熟度和验证成本。",
  },
  {
    heading: "编辑提示",
...
const signalSeries = [42, 49, 47, 61, 58, 71, 76];

function SignalChart() {
  const width = 320;
  const height = 150;
  const padding = 18;
  const maxValue = Math.max(...signalSeries);
  const minValue = Math.min(...signalSeries);
  const stepX = (width - padding * 2) / (signalSeries.length - 1);

  const points = signalSeries
    .map((value, index) => {
...
function SectionTitle({ kicker, title, meta }: { kicker: string; title: string; meta?: string }) {
  return (
...
export function PreviewDashboard({ health, currentPath }: PreviewDashboardProps) {
  const currentLabel =
    {
      "/": "总览视角",
      "/stories": "审核视角",
      ...
      <section className="preview-grid">
        ...
        <article className="panel publish-panel">
          ...
          <div className="runway">
            {publishWindows.map((window, index) => (
